	...
	playDatas.Datas=make(map[string] playData,500)
	 for {
		 ...
		 if playDatas.Datas[record[1]].Count==0{
		 	...
		 	score+=playDatas.Datas[record[1]].Score/playDatas.Datas[record[1]].Count+1
		 	playDatas.Datas[record[1]]=playData{Count: playDatas.Datas[record[1]].Count+1,Score:score }
		 }
	 }
	 return &playDatas
}
