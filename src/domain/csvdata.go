...
func (r *RankingDatas) SortRankingScore() *[]int {
	keys := make([]int, len(r.Datas), len(r.Datas))

	i := 0

	for key := range r.Datas {
		keys[i] = key
		i++
	}
	sort.Ints(keys)
...
//func (r *RankingDatas) PrintResults(scores []int) {
//	for index := len(scores) - 1; index > len(scores)-11; index-- {
//		fmt.Print(scores[index])
//		fmt.Println(r.Datas[scores[index]])
//	}
//}
